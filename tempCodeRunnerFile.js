...
function showConfirmationModal(title, message, callback) {
  ...
  actionBtn.className =
    title.includes("Hapus") || title.includes("Logout")
      ? "btn btn-danger"
      : "btn btn-primary";
  actionBtn.textContent = title.includes("Hapus")
    ? "Ya, Hapus"
    : "Ya, Lanjutkan";
...
function attachEventListeners() {
  ...
  document
    ...
    ?.addEventListener("submit", handleGuestbookSubmit);
  document
  ...
  document
    ...
    ?.addEventListener("input", loadAllEntries);
  document
    .getElementById("filterStatus")
    ?.addEventListener("change", loadAllEntries);
  document
...
function handleLogin(e) {
  ...
  if (user && user.status === "active") {
    ...
    showAlert(
      "loginAlert",
      "Email atau password salah, atau akun Anda belum aktif!",
      "error"
    );
  }
}
...
function showMainApp() {
  ...
  document.getElementById("navUserAvatar").textContent = currentUser.name
    .charAt(0)
    .toUpperCase();
  ...
  navigateTo(
    "dashboard",
    document.querySelector("#navMenu .nav-item:nth-child(2)")
  );
}
...
window.navigateTo = function (sectionId, targetElement) {
  ...
  if (targetElement) {
    ...
    const defaultActive = document.querySelector(
      `.nav-menu button[onclick*="'${sectionId}'"]`
    );
...
function handleGuestbookSubmit(e) {
  ...
  showAlert(
    ...
    "Entri berhasil disimpan!" +
      (entry.status === "pending" ? " Menunggu approval Admin." : ""),
    "success"
  );
  this.reset();
...
function loadAllEntries() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align: center; color: var(--color-text-muted);">Tidak ada entri</td></tr>';
    return;
  }
    ...
    .map(
      (entry) => `
        <tr>
            ...
            <td>${entry.purpose.substring(0, 50)}${
        ...
        entry.status === "pending" ? "Pending" : "Approved"
            ...
            <td>
                <div class="action-btns">
                    <button class="btn btn-primary btn-sm" onclick="viewEntry(${
                      entry.id
                    })">Lihat</button>
                    ${
...
function loadMyEntries() {
  ...
  const myEntries = guestbookEntries
    .filter((e) => e.userId === currentUser.id)
    .sort((a, b) => new Date(b.date) - new Date(a.date));

  if (myEntries.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align: center; color: var(--color-text-muted);">Belum ada entri</td></tr>';
    return;
  }

  tbody.innerHTML = myEntries
    .map(
      (entry) => `
        <tr>
            <td>${formatDate(entry.date)}</td>
        ...
        entry.status === "pending" ? "Pending" : "Approved"
            ...
            <td>
                <div class="action-btns">
                    ...
                    <button class="btn btn-secondary btn-sm" onclick="editEntry(${
                      entry.id
                    })">Edit</button>
...
function loadRecentEntries() {
  ...
  if (recent.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align: center; color: var(--color-text-muted);">Belum ada entri</td></tr>';
    return;
  }
    ...
    .map(
      (entry) => `
        <tr>
            ...
            <td>${entry.guestName}</td>
            <td>${entry.institution}</td>
            <td>${entry.purpose.substring(0, 30)}${
        entry.purpose.length > 30 ? "..." : ""
      }</td>
            <td><span class="badge badge-${entry.status}">${
        entry.status === "pending" ? "Pending" : "Approved"
      }</span></td>
...
function updateDashboard() {
  ...
  document.getElementById("todayEntries").textContent = guestbookEntries.filter(
...
window.viewEntry = function (id) {
  ...
  const content = `
        <div style="line-height: 1.8; color: var(--color-text-muted);">
            <p style="color: var(--color-text);"><strong>Tanggal:</strong> ${formatDate(
              entry.date
            )}</p>
            <p style="color: var(--color-text);"><strong>Nama Tamu:</strong> ${
              entry.guestName
            }</p>
            <p style="color: var(--color-text);"><strong>Email:</strong> ${
              entry.email
            }</p>
            <p style="color: var(--color-text);"><strong>Telepon:</strong> ${
              entry.phone
            }</p>
            <p style="color: var(--color-text);"><strong>Institusi:</strong> ${
              entry.institution
            }</p>
            <p style="color: var(--color-text);"><strong>Bertemu Dengan:</strong> ${
              entry.meetWith
            }</p>
            <p style="color: var(--color-text);"><strong>Status:</strong> <span class="badge badge-${
              entry.status
            }">${entry.status === "pending" ? "Pending" : "Approved"}</span></p>
            <p style="color: var(--color-text);"><strong>Dibuat Oleh:</strong> ${
              entry.userName
            }</p>
            <div style="margin-top: 1rem; padding: 1rem; border: 1px solid rgba(0, 242, 255, 0.1); border-radius: 8px;">
                ...
                <p style="white-space: pre-wrap; color: var(--color-text);">${
                  entry.purpose
                }</p>
            </div>
        </div>
...
window.deleteEntry = function (id) {
  showConfirmationModal(
    "Hapus Entri",
    "Anda yakin ingin menghapus entri buku tamu ini?",
    () => {
...
window.approveEntry = function (id) {
  ...
  if (entry) {
    showConfirmationModal(
      "Approve Entri",
      `Yakin ingin menyetujui entri tamu dari ${entry.guestName}?`,
      () => {
    ...
    .map(
      (user) => `
        ...
        user.role === "admin" ? "Admin" : "User"
            ...
            }">${user.status === "active" ? "Aktif" : "Nonaktif"}</span></td>
            <td>
                <div class="action-btns">
                    ${
...
window.promoteToAdmin = function (id) {
  ...
  if (user) {
    showConfirmationModal(
      "Promosikan User",
      `Yakin ingin menjadikan user ${user.name} sebagai admin? User akan memiliki akses penuh.`,
      () => {
        user.role = "admin";
        saveUsers();
        loadUsers();
...
window.demoteToUser = function (id) {
  ...
  if (user) {
    showConfirmationModal(
      "Turunkan Role",
      `Yakin ingin menurunkan role admin ${user.name} menjadi user biasa?`,
      () => {
        user.role = "user";
        saveUsers();
        loadUsers();
...
window.exportToPDF = function () {
  showAlert(
    "mainAlert",
    'Fitur export PDF menggunakan fitur cetak browser. Silakan pilih "Simpan sebagai PDF" di dialog cetak.',
    "info"
  );
  setTimeout(() => {
    printEntries();
